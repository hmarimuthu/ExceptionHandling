...
/**
 * Will maintain a list of active and not closed browsers for each test bed. In
 * case, if a test case didn't pass, this list of browser will be close and quit
 * at OnFinish of TestListeners Otherwise, number of browsers will be hanging.
 * 
...
public class CurrentDrivers {
	...
	/**
	 * This method is used for debugging purpose. Prints list of drivers on
	 * testBedName basis
	 */
	public void printDriverList() {
		if (driverMap != null) {
			for (String key : driverMap.keySet()) {
				log.info(" ==============================================");
				log.info("    TestBed Name : " + key);
				int i = 0;
				for (Object driver : driverMap.get(key)) {
					i++;
					log.info(i + ":" + driver.toString());
