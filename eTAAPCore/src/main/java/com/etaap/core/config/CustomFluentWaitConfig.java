 ...
 *
 */
public class CustomFluentWaitConfig implements ExceptionListener {
	 ...
	 * @param driver
	 */
	@HandleException(expected = { TimeoutException.class })
	private void wait(WebDriver driver) {
		try {
			...
			ex.handleit(this, timeoutException);
			return;
		}
	}
	...
	private boolean isPageLoadComplete(WebDriver driver) {
		...
		if (driver.getClass().isInstance(SafariDriver.class)) {
			...
			try {
				...
				// Restoring the interrupted status after catching
