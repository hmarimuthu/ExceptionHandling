package com.etaap.defect.tools.rally;
...
import com.etaap.common.util.TafPassword;
import com.etaap.core.config.RallyConfig;
 ...
 * 
 */
public class Rally implements DefectTool {
	...
	public boolean isDefectLogged(String defectTitle) throws InterruptedException, ExecutionException, IOException {
		...
		Fetch queryFetch = new Fetch(RallyConstants.ID.getValue(), RallyConstants.NAME.getValue(),
				RallyConstants.DESCRIPTION.getValue(), RallyConstants.STATE.getValue(),
				RallyConstants.SCHEDULE_STATE.getValue(), RallyConstants.PRIORITY.getValue(),
				RallyConstants.NOTES.getValue());

		QueryFilter queryFilter = new QueryFilter(RallyConstants.NAME.getValue(), RallyConstants.EQUALS.getValue(),
				defectTitle);
	 ...
	 * @throws IOException
	 */
	private boolean buildQueryResponse(String defectTitle, boolean isDefectLogged, QueryResponse queryResponse)
			throws IOException {

		boolean defectLogged = isDefectLogged;

		if (queryResponse.wasSuccessful()) {

			if (queryResponse.getTotalResultCount() > 0) {

				defectLogged = getLoggedDefect(defectTitle, isDefectLogged, queryResponse);
			} else {
				defectLogged = false;
			}

		} else {
			log.error("The following errors occurred in buildQueryResponse: ");
	...
	private boolean getLoggedDefect(String defectTitle, boolean isDefectLogged, QueryResponse queryResponse) {
		...
		for (JsonElement result : queryResponse.getResults()) {
			...
			log.debug(String.format("\t%s - %s: Priority=%s, State=%s",
					defect.get(RallyConstants.ID.getValue()).getAsString(),
					defect.get(RallyConstants.NAME.getValue()).getAsString(),
					defect.get(RallyConstants.PRIORITY.getValue()).getAsString(),
					defect.get(RallyConstants.STATE.getValue()).getAsString()));
	 ...
	 * @throws IOException
	 */
	private QueryResponse getQueryResponse(String queryType, Fetch queryFetch, QueryFilter queryFilter)
			throws IOException {
	...
	@Override
	public void logDefect(String defectTitle, StringBuilder defectDescription, String defectPriority,
			File fileAttachment) throws IOException {

		JsonObject newDefect = new JsonObject();
		newDefect.addProperty(RallyConstants.NAME.getValue(), defectTitle);
		newDefect.addProperty(RallyConstants.PRIORITY.getValue(),
				defectPriority.equals(RallyConstants.MEDIUM_PRIORITY.getValue())
						? RallyConstants.NORMAL_PRIORITY.getValue() : RallyConstants.LOW_PRIORITY.getValue());
		...
		if (!createResponse.wasSuccessful()) {
			log.error("The following errors occurred in logDefect: ");

			for (String err : createResponse.getErrors()) {
				log.error(RallyConstants.TAB.getValue() + err);
			}
			throw new IOException("Error occured in logDefect");
		} else {
			...
			updateStatus(new StringBuilder("" + new Date()), RallyConstants.OPEN_STATE.getValue(),
					RallyConstants.DEFINED_SCHEDULE_STATE.getValue(), fileAttachment);
		}
	}

	@Override
	public void closeDefect(StringBuilder comments) throws IOException {
		updateStatus(comments, RallyConstants.CLOSED_STATE.getValue(),
				RallyConstants.ACCEPTED_SCHEDULE_STATE.getValue(), null);

	}

	@Override
	public void reOpenDefect(StringBuilder comments, File fileAttachment) throws IOException {
		updateStatus(comments, RallyConstants.OPEN_STATE.getValue(),
				RallyConstants.IN_PROGRESS_SCHEDULE_STATE.getValue(), fileAttachment);

	}

	@Override
	public void updateDefect(StringBuilder comments, File fileAttachment) throws IOException {
		updateStatus(comments, RallyConstants.OPEN_STATE.getValue(),
				RallyConstants.IN_PROGRESS_SCHEDULE_STATE.getValue(), fileAttachment);
	 ...
	 * @throws IOException
	 */
	private void updateStatus(StringBuilder comments, String state, String scheduledState, File fileAttachment)
			throws IOException {
	 ...
	 * @throws IOException
	 */
	private void postUpdates(StringBuilder comments, String state, String scheduledState, String ref)
			throws IOException {
		...
		if (!updateResponse.wasSuccessful()) {
			log.error("The following errors occurred iin postUpdates: ");
			for (String err : updateResponse.getErrors()) {
				log.error(RallyConstants.TAB.getValue() + err);
			}

			throw new IOException("Error in Rally while posting updates");
		}
	}

	private void updateFileAttachment(String defectRef, StringBuilder comments, File fileAttachment)
			throws IOException {
	 ...
	 * @throws IOException
	 */
	private void buildFileAttachment(String defectRef, StringBuilder comments, File fileAttachment,
			String fileBase64String, String userRef) throws IOException {
		...
		CreateRequest attachmentContentCreateRequest = new CreateRequest(RallyConstants.ATTACHMENT_CONTENT.getValue(),
		...
		if (attachmentContentResponse.wasSuccessful()) {
			...
			throw new IOException("Error in Rally while building file with attachment");
		}
	}
	 ...
	 * @throws IOException
	 */
	private void postAttachment(String defectRef, StringBuilder comments, File fileAttachment, String userRef,
			CreateResponse attachmentContentResponse) throws IOException {

		String fileAttachmentContentRef = attachmentContentResponse.getObject().get(RallyConstants.REFERENCE.getValue())
				.getAsString();
		...
		attachmentObject.addProperty(RallyConstants.CONTENT_TYPE.getValue(),
				RallyConstants.IMAGE_CONTENT_TYPE.getValue());
		...
		CreateRequest attachmentCreateRequest = new CreateRequest(RallyConstants.ATTACHMENT.getValue(),
				attachmentObject);
		CreateResponse attachmentResponse = restApi.create(attachmentCreateRequest);

		if (attachmentResponse.wasSuccessful()) {
			log.debug("Attachment  created: "
					+ attachmentResponse.getObject().get(RallyConstants.REFERENCE.getValue()).getAsString());
		} else {
			log.error("The following errors occurred: ");
			for (String err : attachmentContentResponse.getErrors()) {
				log.error(RallyConstants.TAB.getValue() + err);
			}
			throw new IOException("Error in Rally while posting attachment");
		}
	...
	private String getUserRef() throws IOException {
		...
		Fetch userFetch = new Fetch(RallyConstants.USER_NAME.getValue(), RallyConstants.SUBSCRIPTION.getValue(),
				RallyConstants.USER_DISPLAY_NAME.getValue());
		QueryFilter userQueryFilter = new QueryFilter(RallyConstants.USER_NAME.getValue(),
				RallyConstants.EQUALS.getValue(), rallyConfig.getUsername());
