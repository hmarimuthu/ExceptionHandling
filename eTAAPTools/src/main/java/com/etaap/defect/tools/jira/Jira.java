...
import com.etaap.core.config.JiraConfig;
 ...
 * 
 */
public class Jira implements DefectTool {
	...
	public void openConnection() throws URISyntaxException {
		...
		restClient = factory.createWithBasicHttpAuthentication(jiraServerUri, jiraConfig.getUsername(),
				tfp.getDecrypted());
	...
	public boolean isDefectLogged(String defectTitle) throws InterruptedException, ExecutionException, IOException {
		...
		String project = addParams(JiraConstants.PROJECT.getValue(), JiraConstants.EQUAL.getValue(),
				jiraConfig.getProjectName());
		searchQuery.append(project.concat(JiraConstants.AND.getValue()));

		String issueType = addParams(JiraConstants.ISSUE_TYPE.getValue(), JiraConstants.EQUAL.getValue(),
				JiraConstants.BUG.getValue());
		searchQuery.append(issueType.concat(JiraConstants.AND.getValue()));

		String defectSummary = addParams(JiraConstants.SUMMARY.getValue(), JiraConstants.LIKE.getValue(),
				"\"\\\"" + defectTitle.trim() + "\\\"\"");
		...
		if (searchResult.getTotal() > 0) {
			...
			for (Issue issue : iterableIssue) {
				...
				if (defectTitle.equals(issue.getSummary())
	...
	public void logDefect(String defectTitle, StringBuilder defectDescription, String defectPriority,
			File fileAttachment) throws URISyntaxException {

		IssueInputBuilder issueBuilder = new IssueInputBuilder(jiraConfig.getKey(),
				Long.valueOf(JiraConstants.BUG_TYPE_ID.getValue()), defectTitle.trim());
	...
	@Override
	public void reOpenDefect(StringBuilder comments, File fileAttachment) throws URISyntaxException {
		updateTransition(JiraConstants.REOPEN.getValue());
		attachScreenshot(fileAttachment, comments);
	}

	@Override
	public void updateDefect(StringBuilder comments, File fileAttachment) throws URISyntaxException {
	 ...
	 * @throws Exception
	 */
	private void attachScreenshot(File fileAttachment, StringBuilder comments) throws URISyntaxException {
	...
	private void addComments(StringBuilder comments) throws URISyntaxException {
		...
		try {
			...
			log.error("Could not add comment to Jira Issue" + loggedIssue.getKey() + " :: "
	...
	private void addAttachment(File attachmentFile) {
		...
		try {
			...
			Promise<Void> attachementAdded = issueRestClient.addAttachments(loggedIssue.getAttachmentsUri(),
					attachmentFile);
			attachementAdded.claim();
		} catch (Exception exp) {
			log.error("Unable to attach file ==>" + attachmentFile.getName() + " to Jira on issueId ==> "
					+ loggedIssue.getKey() + " :: " + exp);
